            ...
            if ($this->plugin_enabled_from_admin_panel()) {  //proceed only if the plugin is enabled 
                  ...
                  switch ($event) {
                        case 'q_post':
                              ...
                              // LOG
                              file_put_contents('./logs/email.log', 'New group event(' . date("y-m-d"). ')' . "\r\n" , FILE_APPEND);
                              file_put_contents('./logs/email.log', 'Event: ' . json_encode($event) . "\r\n" , FILE_APPEND);
                              file_put_contents('./logs/email.log', 'Parameters: ' . json_encode($params) . "\r\n" , FILE_APPEND);
                              file_put_contents('./logs/email.log', 'Path: ' . qa_q_path($params['postid'], $params['title']) . "\r\n" , FILE_APPEND);
                              file_put_contents('./logs/email.log', 'Emails: ' . json_encode($emails) . "\r\n" , FILE_APPEND);
                              ...
                              for ($i = 0; $i < count($emails); $i++) {
                                    ...
                                    $log = json_encode($arr);
                                    file_put_contents('./logs/email_log.log', 'New Mail to:' . $emails[$i]['email'] . "\r\n" , FILE_APPEND);
                                    file_put_contents('./logs/email_log.log', 'User:' . $emails[$i]['email'] . "\r\n" , FILE_APPEND);
                              ...
                              file_put_contents('./logs/email.log', 'Event Finished!' . "\r\n\r\n\r\n" , FILE_APPEND);
                              break;
                  } //switch 
            }//if 
      }
...
//qa_db_notificaton_emails_selectspec
      ...
      function category_email_send_email($params) {
            ...
            // LOGS
            file_put_contents('./logs/email.log', 'Final Parameters: ' . json_encode($params) . "\r\n" , FILE_APPEND);
            file_put_contents('./logs/email.log', 'Mail Object: ' . json_encode($mailer) . "\r\n" , FILE_APPEND);

            if (qa_opt('smtp_active')) {
                  file_put_contents('./logs/email.log', 'Using SMTP. Checking verification...' . "\r\n" , FILE_APPEND);

                  $mailer->IsSMTP();
                  $mailer->Host = qa_opt('smtp_address');
                  file_put_contents('./logs/email.log', 'SMTP Address: ' . qa_opt('smtp_address') . "\r\n" , FILE_APPEND);
                  $mailer->Port = qa_opt('smtp_port');
                  file_put_contents('./logs/email.log', 'SMTP Address: ' . qa_opt('smtp_port') . "\r\n" , FILE_APPEND);

                  if (qa_opt('smtp_secure')) $mailer->SMTPSecure = qa_opt('smtp_secure');
                  file_put_contents('./logs/email.log', 'SMTP secure: ' . qa_opt('smtp_secure') . "\r\n" , FILE_APPEND);

                  if (qa_opt('smtp_authenticate')) {
                        $mailer->SMTPAuth = true;
                        $mailer->Username = qa_opt('smtp_username');
                        $mailer->Password = qa_opt('smtp_password');
                        file_put_contents('./logs/email.log', 'SMTP Authentication Username:' . qa_opt('smtp_username') . "\r\n" , FILE_APPEND);
                        file_put_contents('./logs/email.log', 'SMTP Authentication Password:' . qa_opt('smtp_password') . "\r\n" , FILE_APPEND);
                  }else
                    file_put_contents('./logs/email.log', 'No SMTP Authentication!' . "\r\n" , FILE_APPEND);
            }
            
            file_put_contents('./logs/email.log', 'Final Mail Object: ' . json_encode($mailer) . "\r\n" , FILE_APPEND);
