...
// Justina - start - added new variable for the maximum lenght of a password
if (!defined('QA_MAX_PASSWORD_LEN')) {
	define('QA_MAX_PASSWORD_LEN', 20);
}
//Justina - end
...
function qa_password_validate($password, $olduser = null)
{
	...
	if (!isset($error)) {
		...
		//Justina - start - added validations for a stronger password setup
		else {
			$maxpasslen = max(QA_Max_PASSWORD_LEN, 20);
			if (qa_strlen($password) > $maxpasslen)
				$error = qa_lang_sub('users/password_max', $maxpasslen);
			else {
				if (!preg_match("#[0-9]+#", $password))
					$error = qa_lang_sub('users/password_number', '');
				else {
					if (!preg_match("#[a-z]+#", $password))
						$error = qa_lang_sub('users/password_char_lower', '');
					else {
						if (!preg_match("#[A-Z]+#", $password))
							$error = qa_lang_sub('users/password_char_upper', '');
						else {
							if (!preg_match("#\W+#", $password))
								$error = qa_lang_sub('users/password_char_special', '');
						}
					}
				}
			}
